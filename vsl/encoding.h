...
inline auto is_ascii(std::string_view str) -> bool
{
    for (char c : str)
    {
        if (static_cast<unsigned char>(c) >= detail::LEAST_NONASCII_SYMBOL) return false;
    }
    return true;
}
...
inline auto to_encoding(std::string& out, std::string_view str, const Encoding& encoding, char repl_char = '?') -> void
{
    for (u32 cp : una::ranges::utf8_view(str))
    {
        ...
        else if (auto it = encoding.to_encoding_map.find(cp); it != encoding.to_encoding_map.end())
        {
...
inline auto from_encoding(std::string& out, std::string_view str, const Encoding& encoding) -> void
{
    for (char c : str)
    {
        ...
        {
            out.append(encoding.from_encoding_map[byte - detail::LEAST_NONASCII_SYMBOL]);
        }
    }
}
...
using ToEncodingMap = std::unordered_map<u32, std::string_view>;
...
struct cp1251_t
{
    static inline const ToEncodingMap to_encoding_map = {
        {0x0402, "\x80"},
        {0x0403, "\x81"},
        {0x201A, "\x82"},
        {0x0453, "\x83"},
...
inline const auto cp1251 = detail::cp1251_t{};
inline const auto cp866 = detail::cp866_t{};
inline const auto translit_ru = detail::translit_ru_t{};
