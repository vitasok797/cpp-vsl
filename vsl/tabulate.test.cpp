#include "tabulate.h"

#include <gmock/gmock.h>
#include <gtest/gtest.h>

#include <fmt/format.h>

#include <string>
#include <vector>

namespace vsl::tabulate::test
{

static const auto header = {"ID", "Name", "Type"};
static const auto empty_header = std::vector<std::string>{};
static const auto item_to_row = [](int x) -> vsl::tabulate::TableRow
{
    auto row = vsl::tabulate::TableRow{};
    row.push_back(fmt::format("{}", x));
    row.push_back(fmt::format("{}", x * 10));
    row.push_back(fmt::format("{}", x * 100));
    return row;
};

static auto get_table(int row_count, vsl::tabulate::RowBorders row_borders) -> std::string
{
    auto table = vsl::tabulate::Table{};
    for (auto i = 0; i < row_count; ++i)
    {
        table.add_row({fmt::format("A{}", i), fmt::format("B{}", i)});
    }

    vsl::tabulate::format_row_borders(table, row_borders);

    return table.str();
}

TEST(TabulateTest, RowBordersFull)
{
    ASSERT_EQ(get_table(4, vsl::tabulate::RowBorders::ALL), "+----+----+\n"
                                                            "| A0 | B0 |\n"
                                                            "+----+----+\n"
                                                            "| A1 | B1 |\n"
                                                            "+----+----+\n"
                                                            "| A2 | B2 |\n"
                                                            "+----+----+\n"
                                                            "| A3 | B3 |\n"
                                                            "+----+----+");

    ASSERT_EQ(get_table(3, vsl::tabulate::RowBorders::ALL), "+----+----+\n"
                                                            "| A0 | B0 |\n"
                                                            "+----+----+\n"
                                                            "| A1 | B1 |\n"
                                                            "+----+----+\n"
                                                            "| A2 | B2 |\n"
                                                            "+----+----+");

    ASSERT_EQ(get_table(2, vsl::tabulate::RowBorders::ALL), "+----+----+\n"
                                                            "| A0 | B0 |\n"
                                                            "+----+----+\n"
                                                            "| A1 | B1 |\n"
                                                            "+----+----+");

    ASSERT_EQ(get_table(1, vsl::tabulate::RowBorders::ALL), "+----+----+\n"
                                                            "| A0 | B0 |\n"
                                                            "+----+----+");
}

TEST(TabulateTest, RowBordersHeader)
{
    ASSERT_EQ(get_table(4, vsl::tabulate::RowBorders::HEADER), "+----+----+\n"
                                                               "| A0 | B0 |\n"
                                                               "+----+----+\n"
                                                               "| A1 | B1 |\n"
                                                               "| A2 | B2 |\n"
                                                               "| A3 | B3 |\n"
                                                               "+----+----+");

    ASSERT_EQ(get_table(3, vsl::tabulate::RowBorders::HEADER), "+----+----+\n"
                                                               "| A0 | B0 |\n"
                                                               "+----+----+\n"
                                                               "| A1 | B1 |\n"
                                                               "| A2 | B2 |\n"
                                                               "+----+----+");

    ASSERT_EQ(get_table(2, vsl::tabulate::RowBorders::HEADER), "+----+----+\n"
                                                               "| A0 | B0 |\n"
                                                               "+----+----+\n"
                                                               "| A1 | B1 |\n"
                                                               "+----+----+");

    ASSERT_EQ(get_table(1, vsl::tabulate::RowBorders::HEADER), "+----+----+\n"
                                                               "| A0 | B0 |\n"
                                                               "+----+----+");
}

TEST(TabulateTest, RowBordersNone)
{
    ASSERT_EQ(get_table(3, vsl::tabulate::RowBorders::NONE), "+----+----+\n"
                                                             "| A0 | B0 |\n"
                                                             "| A1 | B1 |\n"
                                                             "| A2 | B2 |\n"
                                                             "+----+----+");

    ASSERT_EQ(get_table(2, vsl::tabulate::RowBorders::NONE), "+----+----+\n"
                                                             "| A0 | B0 |\n"
                                                             "| A1 | B1 |\n"
                                                             "+----+----+");

    ASSERT_EQ(get_table(1, vsl::tabulate::RowBorders::NONE), "+----+----+\n"
                                                             "| A0 | B0 |\n"
                                                             "+----+----+");
}

TEST(TabulateTest, CreateTable)
{
    auto items = {1, 2, 3, 4, 5};
    ASSERT_EQ(vsl::tabulate::create_table(header, items, item_to_row), "+----+------+------+\n"
                                                                       "| ID | Name | Type |\n"
                                                                       "+----+------+------+\n"
                                                                       "| 1  | 10   | 100  |\n"
                                                                       "| 2  | 20   | 200  |\n"
                                                                       "| 3  | 30   | 300  |\n"
                                                                       "| 4  | 40   | 400  |\n"
                                                                       "| 5  | 50   | 500  |\n"
                                                                       "+----+------+------+");
}

TEST(TabulateTest, CreateTableNoHeader)
{
    auto items = {1, 2, 3, 4, 5};
    ASSERT_EQ(vsl::tabulate::create_table(empty_header, items, item_to_row), "+---+----+-----+\n"
                                                                             "| 1 | 10 | 100 |\n"
                                                                             "| 2 | 20 | 200 |\n"
                                                                             "| 3 | 30 | 300 |\n"
                                                                             "| 4 | 40 | 400 |\n"
                                                                             "| 5 | 50 | 500 |\n"
                                                                             "+---+----+-----+");
}

TEST(TabulateTest, CreateTableOneRow)
{
    auto items = {1};
    ASSERT_EQ(vsl::tabulate::create_table(header, items, item_to_row), "+----+------+------+\n"
                                                                       "| ID | Name | Type |\n"
                                                                       "+----+------+------+\n"
                                                                       "| 1  | 10   | 100  |\n"
                                                                       "+----+------+------+");
}

TEST(TabulateTest, CreateTableOneRowNoHeader)
{
    auto items = {1};
    ASSERT_EQ(vsl::tabulate::create_table(empty_header, items, item_to_row), "+---+----+-----+\n"
                                                                             "| 1 | 10 | 100 |\n"
                                                                             "+---+----+-----+");
}

TEST(TabulateTest, CreateTableVariableCellCount)
{
    auto items = {1, 2, 3};
    auto item_to_row_variable = [](int x)
    {
        return vsl::tabulate::TableRow(x, fmt::format("{}", x));  //
    };
    ASSERT_EQ(vsl::tabulate::create_table(header, items, item_to_row_variable), "+----+------+------+\n"
                                                                                "| ID | Name | Type |\n"
                                                                                "+----+------+------+\n"
                                                                                "| 1  |      |      |\n"
                                                                                "| 2  | 2    |      |\n"
                                                                                "| 3  | 3    | 3    |\n"
                                                                                "+----+------+------+");
}

}  // namespace vsl::tabulate::test
