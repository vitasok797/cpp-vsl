...
{
    EXPECT_TRUE(vsl::is_valid_json_string(PERSON_STR));
    EXPECT_FALSE(vsl::is_valid_json_string(PERSON_STR_BAD));
}
...
{
    {
        ...
        EXPECT_THROW(json["name"].get<int>(), vsl::json_type_error);
        EXPECT_THROW(json.at("unknown"), vsl::json_out_of_range);
    }
    {
        ...
        EXPECT_THROW(json["arr_partial"].get<tuple_t>(), vsl::json_out_of_range);
    }
}
...
{
    const auto json_partial = Json::parse(PERSON_STR_PARTIAL);
    EXPECT_THROW(json_partial.get<PersonInline>(), vsl::json_out_of_range);
    EXPECT_THROW(json_partial.get<Person>(), vsl::json_out_of_range);
    EXPECT_THROW(json_partial.get<PersonStrictInline>(), vsl::json_out_of_range);
    EXPECT_THROW(json_partial.get<PersonStrict>(), vsl::json_out_of_range);
}
...
TEST(JsonTest, GetStructExtraKeyError)
{
    ...
    EXPECT_THROW(json_extra.get<PersonStrictInline>(), vsl::json_out_of_range);
    EXPECT_THROW(json_extra.get<PersonStrict>(), vsl::json_out_of_range);
    ...
    EXPECT_THROW(json_partial_extra.get<PersonStrictInlineWithDefault>(), vsl::json_out_of_range);
    EXPECT_THROW(json_partial_extra.get<PersonStrictWithDefault>(), vsl::json_out_of_range);
}

TEST(JsonTest, GetStructTypeError)
{
    const auto json = Json::parse(R"({"name":1,"age":20})");
    EXPECT_THROW(json.get<PersonInline>(), vsl::json_type_error);
    EXPECT_THROW(json.get<Person>(), vsl::json_type_error);
    EXPECT_THROW(json.get<PersonStrictInline>(), vsl::json_type_error);
    EXPECT_THROW(json.get<PersonStrict>(), vsl::json_type_error);
}
...
{
    {
        ...
        EXPECT_THROW(vsl::detail::check_json_for_extra_keys(json, json_partial), vsl::json_out_of_range);
    }
    {
...
TEST(JsonTest, GetStrictStruct)
{
    ...
    EXPECT_THROW(vsl::get_strict_struct_from_json<Person>(json_wrong_type), vsl::json_type_error);
    EXPECT_EQ(vsl::try_get_strict_struct_from_json<Person>(json_wrong_type), std::nullopt);
}

TEST(JsonTest, JsonToPrettyLine)
{
    const auto json = Json::parse(PERSON_STR);
    EXPECT_EQ(vsl::json_to_pretty_line(json), PERSON_STR_SORTED_PRETTY);

    const auto json_ord = OrderedJson::parse(PERSON_STR);
    EXPECT_EQ(vsl::json_to_pretty_line(json_ord), PERSON_STR_PRETTY);

    const auto json_non_ascii = Json::parse(R"({"val":"тест"})");
    EXPECT_EQ(vsl::json_to_pretty_line(json_non_ascii), R"({ "val": "тест" })");
    EXPECT_EQ(vsl::json_to_pretty_line(json_non_ascii, vsl::EnsureAscii::YES),
              R"({ "val": "\u0442\u0435\u0441\u0442" })");
}
...
TEST(JsonTest, Enum)
{
    ...
    EXPECT_THROW(json["val"].get<DemoEnum>(), vsl::json_type_error);
